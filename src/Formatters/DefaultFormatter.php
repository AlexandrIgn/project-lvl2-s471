...
namespace DifferenceCalculator\Formatters\DefaultFormatter;

function getDefaultFormat($ast, $depth = 0)
{
    ...
    $result = array_reduce($ast, function ($acc, $node) use ($depth, $spaces) {
        if ($node['type'] === 'unchanged') {
            ...
            $acc[] = $spaces . "    {$node['key']}: " . getDefaultFormat($node['children'], $depth + 1);
        }
        return $acc;
    }, []);
    $strResult = implode("\n", $result);
